...
def plot_results(res_time):
    ...
    for i in res_time.keys():
        lengs.append(int(i))
    colors = {'recur_leven' : 'red', 'matr_leven' : 'green', 'recur_matr_leven' : 'blue', 'damer_leven' : 'yellow', 'damer_leven_matr' : 'orange'}
...
def time_analise():
    ...
    for leng in lens:
        print("leng = ", leng)
        time_ = {'recur_leven' : 0.0, 'matr_leven' : 0.0, 'recur_matr_leven' : 0.0, 'damer_leven' : 0.0, 'damer_leven_matr' : 0.0}
        for i in range(times):
            ...
            if leng < 15:
                ...
                time_['recur_leven'] += (stop - start)
            else:
                time_['recur_leven'] = None
            ...
            if leng < 15:
                ...
                time_['damer_leven'] += (stop - start)
            else:
                time_['damer_leven'] = None
            ...
            time_['damer_leven_matr'] += (stop - start)
        for i in time_.keys():
            if time_[i] != None:
                time_[i] /= times
        res_time[leng] = time_
...
for i in res_time.keys():
    print("for leng = ", i, ":")
    print("\t\t\t", res_time[i])
plot_results(res_time)'''


s1 = 'abcd' * 1000000000#input("s1: ")
s2 = 'efgh' * 1000000000#input("s2: ")
M = len(s1)
N = len(s2)
if (N > M):
    M, N = N, M
...
lev_rec(M,N,s1,s2)#print("1. lev_rec = ", lev_rec(M,N,s1,s2))
stop = time.process_time_ns()#time.perf_counter()#time.clock()
print("proc. time = ", stop - start)

#2.
start = time.process_time_ns()#time.perf_counter()#time.clock()
matr_lev(M,N,s1,s2)#print("2. lev_matr = ", matr_lev(M,N,s1,s2))
stop = time.process_time_ns()#time.perf_counter()#time.clock()
print("proc. time = ",stop - start)
...
rec_matr_lev(Matr, M, N, s1, s2)#print("3. rec_lev_matr = ", rec_matr_lev(Matr, M, N, s1, s2))
...
start = time.process_time_ns()#start = time.perf_counter()#time.clock()
damer_leven(M, N, s1, s2)#print("4. damer_leven = ", damer_leven(M, N, s1, s2))
stop = time.process_time_ns()#stop = time.perf_counter()#time.clock()
print("proc. time = ", stop - start)
#5.
Matr = [[sys.maxsize] * (N + 1) for i in range (M + 1)]
start = time.process_time_ns()#start = time.perf_counter()#time.clock()
damer_leven_opt(M, N, s1, s2)#print("5. damer_leven_opt = ", damer_leven_opt(M, N, s1, s2))
stop = time.process_time_ns()#stop = time.perf_counter()#time.clock()
print("proc. time = ", stop - start)
